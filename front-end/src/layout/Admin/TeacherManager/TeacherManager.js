...
import {
    Button, Table, Flex, Breadcrumb, Badge, Modal, Form, Input, Upload,
    Spin, Select, Image
} from 'antd';
...
function TeacherManager() {
    ...
    const columns = [
        ...
        },
        {
            title: 'Giới tính',
            dataIndex: 'gender',
            render: (gender) => <Badge color={gender === "Nam" ? "#1890ff" : "#f759ab"} count={gender} />,
            filters: [
                { text: 'Nam', value: 'Nam' },
                { text: 'Nữ', value: 'Nữ' },
            ],
    ...
    return (
        <Flex className="TeacherManager" vertical gap={20} style={{ position: "relative" }}>
            ...
            <Table rowSelection={rowSelection} columns={columns} dataSource={teachers} bordered />
            <Modal
                ...
                centered
            >
                <p>{selectedRowKeys.length} giảng viên sẽ bị xoá vĩnh viễn. Bạn có chắc không?</p>
            </Modal>
            <Modal
                open={open}
                ...
                centered
            >
                <Form name="teacher_form" onFinish={onFinish} initialValues={{ gender: "Nam" }}>
                    <Form.Item name="full_name" rules={[{ required: true, message: 'Vui lòng nhập họ tên!' }]}>
                        <Input prefix={<SmileOutlined />} placeholder="Họ và tên" allowClear />
                    </Form.Item>
